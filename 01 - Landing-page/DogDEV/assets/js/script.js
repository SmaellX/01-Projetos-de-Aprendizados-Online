...
let testimonials = [
  ...
  { quote: '"DogDev usa ingredientes simples e limpos cem seus pratos."', origin: "fox.svg" },
  { quote: '"Vejo você [João] como um verdadeiro héroi."', origin: "sharktank.svg" },
];
...
for (let tindex in testimonials) {
  ...
  img.setAttribute(
    "src", "./assets/images/" + testimonials[parseInt(tindex)].origin);
  img.style.cursor = "pointer";
  img.addEventListener("click", () => fillTestimonial(parseInt(tindex)));
  testimonialIcons.appendChild(img);
}

let currenTestimonial = 0;
let testimonialTimer;

const fillTestimonial = (index) => {
  clearTimeout(testimonialTimer);
  currenTestimonial = index;
  testimonialQuote.innerHTML = testimonials[currenTestimonial].quote;
  let imgs = testimonialIcons.querySelectorAll("img");
  for (let img of imgs) img.style.opacity = 0.2;
  imgs[currenTestimonial].style.opacity = 1;
  testimonialTimer = setTimeout(nextTestimonial, 3000);
};

const nextTestimonial = () => {
  if (testimonials[currenTestimonial + 1]){
    fillTestimonial(currenTestimonial + 1);
  }else {
    fillTestimonial (0);
  }
}
